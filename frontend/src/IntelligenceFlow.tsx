...
export function IntelligenceFlow({ entry }: { entry: Entry | null }) {
  ...
  return <figure className={`intelligence-flow ${paused ? 'paused' : ''}`} aria-label="Flujo de priorización de VulnSOC">
    ...
    <svg viewBox="0 0 580 286" role="img" aria-labelledby={id}>
      <title id={id}>NVD, CISA KEV, EPSS y activos convergen en VulnSOC para determinar la prioridad contextual. {entry ? `Referencia guardada: ${entry.cve_id}, ${priority}, ${entry.score_mostrado} de 100.` : 'Ejemplo ilustrativo: prioridad crítica, 100 de 100.'}</title>
      ...
      <g className={`flow-result ${severity(priority)}`}><rect x="414" y="99" width="158" height="89" rx="10"/><text x="429" y="121" className="flow-caption">PRIORIDAD CONTEXTUAL</text><text x="429" y="144" className="flow-priority">{priority}</text><text x="429" y="172" className="flow-score">{entry?.score_mostrado ?? 100}<tspan className="flow-caption"> /100</tspan></text></g>
    </svg>
    <div className="flow-mobile"><div className="mobile-sources">{[['NVD','Severidad · CVSS'],['CISA KEV','Explotación conocida'],['EPSS','Probabilidad'],['ACTIVOS','Contexto del entorno']].map(([name,caption])=><div key={name}><b>{name}</b><small>{caption}</small></div>)}</div><div className="mobile-flow-output"><div className="mobile-engine"><b>VulnSOC</b><small>Scoring contextual</small></div><span className="mobile-connector" aria-hidden="true">→</span><div className="mobile-result" data-priority={severity(priority)}><small>Prioridad contextual</small><b>{priority}</b><span>{entry?.score_mostrado??100}<small>/100</small></span></div></div></div>
    <div className="flow-note"><span className="flow-note-mark"/><span>{entry ? `Referencia local · ${entry.cve_id}` : 'Ejemplo ilustrativo · CRÍTICA, 100/100'}</span><small>Esquema del proceso, sin consultas en directo.</small></div>
  </figure>;
}
