...
export function ScoreBreakdown({entry,onOpen}:{entry:Entry|null;onOpen?:()=>void}){
 ...
 const inventoryLabel=(name:string)=>{
  if(!entry||!['Inventario · sin ajuste','Contexto de activo compatible'].includes(name))return name;
  const state=entry.score.contexto_inventario;
  if(state==='sin_inventario')return 'Inventario · sin ajustar';
  if(state==='sin_coincidencias')return 'Inventario · sin coincidencias';
  if(state==='version_compatible')return 'Contexto de activo · compatible';
  if(state==='pendiente_verificacion')return entry.equipos_afectados.length>0&&entry.equipos_afectados.every(asset=>asset.estado==='fuera_de_rango')
   ?'Inventario · versión no afectada':'Inventario · pendiente de verificación';
  return name;
 };
 // The engine omits the zero-point inventory factor when no inventory is configured.
 // Include its known state only in this view; never modify the saved factors or score.
 const displayFactors=entry?.score.contexto_inventario==='sin_inventario'&&!factors.some(f=>f.factor.startsWith('Inventario')||f.factor.startsWith('Contexto de activo'))
  ?[...factors,{factor:'Inventario · sin ajuste',puntos:0,detalle:'No hay activos configurados. No se aplica ajuste por inventario.'}]:factors;
 let accumulated=0;
 const rows=displayFactors.map(f=>{const start=accumulated;accumulated+=f.puntos;return {...f,start,end:accumulated};});
