...
describe('importaciones y referencias',()=>{
 ...
 it('acepta el score único sin el campo capado heredado',()=>{const {score_mostrado,...rest}=entry;const {score_mostrado:legacy,...score}=entry.score;const parsed=entrySchema.parse({...rest,score_interno:255,score:{...score,score_interno:255}});expect(parsed.score_interno).toBe(255);expect(parsed.score_mostrado).toBeUndefined();});
