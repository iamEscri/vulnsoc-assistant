...
test('navigation, Sigma, PDF and reduced motion',async({page})=>{
 ...
 await page.getByRole('button',{name:'Metodología',exact:true}).click();await expect(page.getByRole('heading',{name:'Acerca de VulnSOC'})).toBeVisible();
});
...
test('explanatory flow can pause and respects reduced motion without fabricating analysis',async({page})=>{
 ...
 await expect(flow.getByText('Ejemplo ilustrativo · CRÍTICA, 100/100',{exact:true})).toBeVisible();
 await expect(page.locator('.internal-number')).toHaveText('—pts');
 ...
 await expect(page.locator('.orbit-value')).toHaveAttribute('stroke-dasharray','100 100');
 await page.getByRole('button',{name:'Vista general',exact:true}).click();await expect(flow.getByText('Referencia local · CVE-2021-44228',{exact:true})).toBeVisible();await expect(flow.getByText('Ejemplo ilustrativo · CRÍTICA, 100/100',{exact:true})).toHaveCount(0);
});
